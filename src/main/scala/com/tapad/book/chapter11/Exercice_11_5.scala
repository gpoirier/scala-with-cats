...
import Exercice_11_4.{ GCounter, BoundedSemiLattice }
import cats.instances.list._
import cats.syntax.semigroup._
import cats.syntax.foldable._

object Exercice_11_5 {
  ...
  implicit object MapKeyValueStore extends KeyValueStore[Map] {
    type F[K, V] = Map[K, V]

    def put[K, V](f: F[K, V])(k: K, v: V): F[K, V] = f + (k -> v)
    def get[K, V](f: F[K, V])(k: K): Option[V] = f.get(k)
    def values[K, V](f: F[K, V]): List[V] = f.values.toList
  }
